...
# Modify the transition probabilities and rewards for the Random Walk problem
# Adjust the transition probabilities: 20% left, 80% right
# Adjust rewards: +1 if terminates on the extreme right, 0 otherwise
def temporal_difference(values, alpha=0.1, batch=False):
    ...
    while True:
        old_state = state
        # Adjust transition probabilities: 20% left, 80% right
        if np.random.choice([ACTION_LEFT, ACTION_RIGHT], p=[0.2, 0.8]) == ACTION_LEFT:
            state -= 1
        else:
            state += 1
        # Adjust rewards: +1 if terminates on the extreme right, 0 otherwise
