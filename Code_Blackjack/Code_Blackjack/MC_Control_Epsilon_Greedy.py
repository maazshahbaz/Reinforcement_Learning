...
def make_epsilon_greedy_policy(Q, epsilon, nA):
    ...
    def policy_fn(observation):
        A = np.ones(nA, dtype=float) * epsilon / nA # make sure sum(A) = 1
        best_action = np.argmax(Q[observation])
        A[best_action] += (1.0 - epsilon)
        return A # A=[... epsilon/nA, epsilon/nA + 1 - epsilon, epsilon/nA ...]
    return policy_fn
...
def mc_control_epsilon_greedy(env, num_episodes, discount_factor=1.0, epsilon=0.1):
    ...
    return Q, policy
Q, policy = mc_control_epsilon_greedy(env, num_episodes=500000, epsilon=0.9)
